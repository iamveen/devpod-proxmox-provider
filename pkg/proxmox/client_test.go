...
func TestGetNodeStorage(t *testing.T) {
	...
	storages, err := client.GetNodeStorage(t.Context(), "pve1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(storages) != 1 || storages[0].Storage != "local-lvm" {
		t.Errorf("expected local-lvm, got %+v", storages)
	}
}
...
func TestGetNodeNetworks(t *testing.T) {
	...
	networks, err := client.GetNodeNetworks(t.Context(), "pve1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(networks) != 1 || networks[0].Iface != "vmbr0" {
		t.Errorf("expected vmbr0, got %+v", networks)
	}
}
...
func TestCreateVM(t *testing.T) {
	server, client := newTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/qemu") {
			t.Errorf("expected /qemu path, got %s", r.URL.Path)
		}
		body, _ := io.ReadAll(r.Body)
		params, _ := url.ParseQuery(string(body))
		if params.Get("vmid") != "9000" {
			t.Errorf("expected vmid 9000, got %s", params.Get("vmid"))
		}
		if params.Get("memory") != "2048" {
			t.Errorf("expected memory 2048, got %s", params.Get("memory"))
		}
		json.NewEncoder(w).Encode(map[string]string{
			"data": "UPID:pve1:00001234:qmcreate:9000:root@pam:",
		})
	})
	defer server.Close()

	upid, err := client.CreateVM(t.Context(), "pve1", proxmox.CreateVMRequest{
		VMID:   9000,
...
func TestConvertToTemplate(t *testing.T) {
	server, client := newTestServer(func(w http.ResponseWriter, r *http.Request) {
		if !strings.Contains(r.URL.Path, "/template") {
			t.Errorf("expected /template path, got %s", r.URL.Path)
		}
		json.NewEncoder(w).Encode(map[string]string{
			"data": "UPID:pve1:00001234:template:9000:root@pam:",
		})
	})
	defer server.Close()

	upid, err := client.ConvertToTemplate(t.Context(), "pve1", 9000)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.HasPrefix(upid, "UPID:") {
...
func TestDownloadURL(t *testing.T) {
	server, client := newTestServer(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		params, _ := url.ParseQuery(string(body))
		if !strings.Contains(params.Get("url"), "ubuntu") {
			t.Errorf("expected ubuntu URL, got '%s'", params.Get("url"))
		}
		if params.Get("filename") != "ubuntu.img" {
			t.Errorf("expected filename 'ubuntu.img', got '%s'", params.Get("filename"))
		}
		if params.Get("content") != "import" {
			t.Errorf("expected content 'import', got '%s'", params.Get("content"))
		}
		json.NewEncoder(w).Encode(map[string]string{
			"data": "UPID:pve1:00001234:download:9000:root@pam:",
		})
	})
	defer server.Close()

	upid, err := client.DownloadURL(t.Context(), "pve1", "local-lvm",
		"https://cloud-images.ubuntu.com/ubuntu.img", "ubuntu.img")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.HasPrefix(upid, "UPID:") {
